...
class DateTimeApi
{
	...
	public function dtsToYymmdd($dtsDateTime) {
		...
		$paramXml = 
		  "<parm io='in' comment='1. Input format'>
...
class ListFromApi
{
	...
	public function getNextEntry() {
		...
		$paramXml = 
    "<parm io='out' comment='1. receiver data'>
...
//		$params = $this->DefineUserSpaceParameters($InitSize, $Auth, $InitChar);
    ...
    */
	...
	public function ReadUserSpace($frompos=1, $readlen = 0, $receiveStructure = null ){	
		...
		$dataRead = ' ';		
	    $params[] = ToolkitService::AddParameterChar( 'in', 20,  "User space name and library",
	    											'userspacename',				 
	    											$this->getUSFullName());	
...
class DataQueue{
                                 ...
                                 $QSizeMaxNumEntries=32999,  $QSizeInitNumEntries = 16)
	{
		...
		if(is_integer($QSizeMaxNumEntries)) 
			...
			if( strcmp($QSizeMaxNumEntries, '*MAX16MB') == 0 || 
				strcmp($QSizeMaxNumEntries, '*MAX2GB')== 0)
			$MaxQSize = (string) $QSizeMaxNumEntries;
			
						
		if( $QSizeInitNumEntries > 0 ) $InitEntryies = $QSizeInitNumEntries;
		$AdditionalSetting = sprintf("$KeyedSetting SENDERID(*YES) SIZE(%s %d)",
		                              $MaxQSize, $InitEntryies );
...
class JobLogs {
	...
	public function JobList( $user = NULL, $jobstatus = "*ACTIVE")
	{
		...
		$params[]=$this->ToolkitSrvObj->AddParameterChar('input', 20, 'USER SPACE NAME','userspacename',
		                                         $FullUSName);
...
/*	private function verify_CPFError( $retPgmArr, $functionErrMsg )	 
	{
		...
		$retArr = $retPgmArr['io_param'];		 
		if( isset($retArr) && strcmp($retArr['exceptId'], '0000000')){		    	
		    	$this->CPFErr = $retArr['exceptId'];
	    		$this->ErrMessage = $functionErrMsg . $this->CPFErr;	    	
			return true;  //some problem
	     }
